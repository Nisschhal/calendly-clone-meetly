...
import { Meeting, MeetingStatusEnum } from "../database/entities/meeting.entity"
...
import { validateGoogleTokenService } from "./integration.service"
...
export const getUserMeetingsService = async (
  ...
  if (filter === MeetingFilterEnum.UPCOMING) {
    where.status = MeetingStatusEnum.SCHEDULE
...
export const createMeetBookingForGuestService = async (
  ...
  const meetIntegration = await integrationRepository.findOne({
    where: {
      ...
      app_type:
        IntegrationAppTypeEnum[
          event.locationType as keyof typeof IntegrationAppTypeEnum
        ],
    },
  })
  ...
  if (event.locationType === EventLocationEnumType.GOOGLE_MEET_AND_CALENDER) {
      ...
      switch (calendarType) {
        case IntegrationAppTypeEnum.GOOGLE_MEET_AND_CALENDAR:
          await calendar.events.delete({
            calendarId: "primary",
            eventId: meeting.calenderEventId,
          })
          break
        default:
...
async function getCalendarClient(
  ...
  switch (appType) {
    case IntegrationAppTypeEnum.GOOGLE_MEET_AND_CALENDAR:
      const validToken = await validateGoogleTokenService(
        access_token,
        refresh_token,
        expiry_date
      )
